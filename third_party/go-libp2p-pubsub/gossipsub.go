...
type GossipSubRouter struct {
	...
	heartbeatIntervalUpdate chan time.Duration
...
func (gs *GossipSubRouter) Attach(p *PubSub) {
	...
	// start the heartbeat
	gs.heartbeatIntervalUpdate = make(chan time.Duration, 1)
	go gs.heartbeatTimer(gs.params.HeartbeatInterval, gs.params.HeartbeatInitialDelay)
...
// UpdateGossipSubParams applies runtime-safe parameters through PubSub's
// serialized event loop. Construction-only fields must remain unchanged.
func (gs *GossipSubRouter) UpdateGossipSubParams(ctx context.Context, params GossipSubParams) error {
	if gs == nil || gs.p == nil {
		return fmt.Errorf("pubsub: router is not attached")
	}
	if err := params.validate(); err != nil {
		return err
	}
	if params.HeartbeatInterval <= 0 || params.GossipFactor < 0 || params.GossipFactor > 1 {
		return fmt.Errorf("pubsub: invalid runtime gossip parameters")
	}
	if ctx == nil {
	...
	select {
	case gs.p.eval <- func() {
		if !sameConstructionParams(gs.params, params) {
			result <- fmt.Errorf("pubsub: construction-only parameter changed")
			return
		}
		gs.params = params
		select {
		case gs.heartbeatIntervalUpdate <- params.HeartbeatInterval:
		default:
			<-gs.heartbeatIntervalUpdate
			gs.heartbeatIntervalUpdate <- params.HeartbeatInterval
		}
		result <- nil
	}:
	case <-ctx.Done():
...
// GossipSubParams returns a serialized snapshot of the active parameters.
func (gs *GossipSubRouter) GossipSubParams(ctx context.Context) (GossipSubParams, error) {
	if gs == nil || gs.p == nil {
		return GossipSubParams{}, fmt.Errorf("pubsub: router is not attached")
	}
	if ctx == nil {
		ctx = context.Background()
	}
	result := make(chan GossipSubParams, 1)
	select {
	case gs.p.eval <- func() { result <- gs.params }:
	case <-ctx.Done():
		return GossipSubParams{}, ctx.Err()
	case <-gs.p.ctx.Done():
		return GossipSubParams{}, gs.p.ctx.Err()
	}
	select {
	case params := <-result:
		return params, nil
...
func sameConstructionParams(left, right GossipSubParams) bool {
	left.HeartbeatInterval = right.HeartbeatInterval
	left.D, left.Dlo, left.Dhi, left.Dout = right.D, right.Dlo, right.Dhi, right.Dout
	left.GossipFactor = right.GossipFactor
	left.FanoutTTL, left.PruneBackoff = right.FanoutTTL, right.PruneBackoff
	return left == right
}
...
func (gs *GossipSubRouter) heartbeatTimer(interval, initialDelay time.Duration) {
	select {
	...
	for {
		...
		case interval = <-gs.heartbeatIntervalUpdate:
			if interval <= 0 {
				return
			}
			if !timer.Stop() {
