...
func runExperiments(cfg ExperimentConfig) ([]Result, error) {
	...
	if len(cfg.PeerCounts) > 0 {
		all := make([]Result, 0, len(cfg.PeerCounts)*cfg.Repetitions*len(implementationNames()))
		for _, peers := range cfg.PeerCounts {
			perSize := cfg
			perSize.PeerCounts = nil
			perSize.Peers = peers
			results, err := runExperimentsSingle(perSize)
			if err != nil {
				return nil, err
			}
	...
	return runExperimentsSingle(cfg)
}

func runExperimentsSingle(cfg ExperimentConfig) ([]Result, error) {
...
func convertMetrics(m implementations.Metrics) ExperimentMetrics {
	...
	for _, trace := range m.FLRoundTrace {
		traces = append(traces, implementationsTrace{Round: trace.Round, Method: trace.Method, ModelOutput: trace.ModelOutput, Current: candidateTrace(trace.Current), Candidate: candidateTrace(trace.Candidate), CurrentOutcome: outcomeTrace(trace.CurrentOutcome), ControlledOutcome: outcomeTrace(trace.ControlledOutcome), CurrentObjective: trace.CurrentObjective, CandidateObjective: trace.CandidateObjective, Accepted: trace.Accepted, RejectionReason: trace.RejectionReason, AggregationParameterChange: trace.AggregationParameterChange, PredictorParameterChange: trace.PredictorParameterChange, DecodedMesh: trace.DecodedMesh, MeshCandidates: trace.MeshCandidates, MeshCandidateObjectives: trace.MeshCandidateObjectives, EffectiveWeights: trace.EffectiveWeights})
	}
	return ExperimentMetrics{CPU: m.CPU, MemoryMB: m.MemoryMB, BytesSent: m.BytesSent, BytesReceived: m.BytesReceived, Published: m.Published, Received: m.Received, Delivered: m.Delivered, Duplicates: m.Duplicates, DeliveryRatio: m.DeliveryRatio, DuplicateRatio: m.DuplicateRatio, LatencyAverage: m.LatencyAverage, LatencyP95: m.LatencyP95, AverageMeshDegree: m.MeshDegree, HeartbeatCount: m.HeartbeatCount, Energy: m.Energy, EnergyPerDeliveredMessage: m.EnergyPerDeliveredMessage, ModeledResourceCostTotal: m.ModeledResourceCostTotal, ModeledResourceCostPerDeliveredMessage: m.ModeledResourceCostPerDeliveredMessage, AverageTrust: m.AverageTrust, MinimumTrust: m.MinimumTrust, TrustVariance: m.TrustVariance, TrustedPeerRatio: m.TrustedPeerRatio, TrustUpdateTime: m.TrustUpdateTime, PeerRankingTime: m.PeerRankingTime, TrustAggregationTime: m.TrustAggregationTime, FLRounds: m.FLRounds, TrainingLoss: m.TrainingLoss, GlobalLoss: m.GlobalLoss, TrainingTime: m.TrainingTime, AggregationTime: m.AggregationTime, TotalRoundTime: m.TotalRoundTime, ParameterChange: m.ParameterChange, Contributors: m.Contributors, Rejected: m.Rejected, LocalLossByRound: m.LocalLossByRound, GlobalLossByRound: m.GlobalLossByRound, ParameterChangeByRound: m.ParameterChangeByRound, TrainingTimeByRound: m.TrainingTimeByRound, AggregationTimeByRound: m.AggregationTimeByRound, EffectiveWeights: m.EffectiveWeights, FLRoundTrace: traces, Available: map[string]bool{"live_network_counters": false, "process_cpu": false, "go_heap_memory": true, "trust_metrics": m.AverageTrust > 0}}
}

func candidateTrace(p fedgreensub.GossipParameters) CandidateTrace {
	return CandidateTrace{MeshDegree: p.MeshDegree, DLow: p.DLow, DHigh: p.DHigh, GossipFactor: p.GossipFactor, HeartbeatInterval: p.HeartbeatInterval.String()}
}

func outcomeTrace(o fedgreensub.CandidateScore) OutcomeTrace {
	return OutcomeTrace{DeliveryRatio: o.DeliveryRatio, DuplicateRatio: o.DuplicateRatio, LatencyCost: o.LatencyCost, EnergyCost: o.EnergyCost}
}
...
func scenarioConfig(cfg ExperimentConfig) ExperimentConfig {
	...
	case "energy-scaling":
		if len(cfg.PeerCounts) == 0 {
			cfg.PeerCounts = []int{10, 25, 50, 100, 200}
		}
	}
	return cfg
}
...
func printSummary(results []Result, cfg ExperimentConfig) {
	if cfg.Scenario == "energy-scaling" {
		printEnergyScalingSummary(results)
		return
	}
...
func printEnergyScalingSummary(results []Result) {
	byPeer := make(map[int]map[string][]Result)
	for _, result := range results {
		if byPeer[result.PeerCount] == nil {
			byPeer[result.PeerCount] = make(map[string][]Result)
		}
		byPeer[result.PeerCount][result.Implementation] = append(byPeer[result.PeerCount][result.Implementation], result)
	}
	peers := make([]int, 0, len(byPeer))
	for peerCount := range byPeer {
		peers = append(peers, peerCount)
	}
	sort.Ints(peers)
	fmt.Println("Modeled resource-cost scaling (not physical energy/Joules)")
	for _, peerCount := range peers {
		baseline := meanResults(byPeer[peerCount]["gossipsub"], func(r Result) float64 { return r.Metrics.ModeledResourceCostTotal })
		fmt.Printf("peers=%d baseline_total=%.6f", peerCount, baseline)
		for _, implementation := range []string{"fedgreen", "trustaware"} {
			value := meanResults(byPeer[peerCount][implementation], func(r Result) float64 { return r.Metrics.ModeledResourceCostTotal })
			reduction := 0.0
			if baseline != 0 {
				reduction = (baseline - value) / baseline * 100
			}
			fmt.Printf(" %s_total=%.6f reduction_vs_baseline=%+.2f%%", implementation, value, reduction)
		}
		fmt.Println()
	}
}

func meanResults(results []Result, value func(Result) float64) float64 {
	if len(results) == 0 {
		return 0
	}
	total := 0.0
